...
def run_multiseed_experiment(
    ...
    return {
        ...
        "ci_95": stats.t.interval(
            0.95, len(final_means) - 1,
            loc=np.mean(final_means),
            scale=stats.sem(final_means)
        ) if len(final_means) > 1 else (np.mean(final_means), np.mean(final_means)),
...
def compute_statistical_tests(results: dict[str, dict], logger: logging.Logger) -> dict:
    ...
    for agent in agents:
        ...
        u_stat, u_pvalue = stats.mannwhitneyu(best_means, other_means, alternative='greater')
        
        # Effect size (Cohen's d)
        pooled_std = np.sqrt((np.var(best_means) + np.var(other_means)) / 2)
        cohens_d = (np.mean(best_means) - np.mean(other_means)) / pooled_std if pooled_std > 0 else 0
...
def print_summary_table(results: dict[str, dict], logger: logging.Logger) -> None:
    ...
    # Sort by mean reward
    sorted_agents = sorted(results.keys(), key=lambda x: results[x]["mean"], reverse=True)
    
    header = f"{'Rank':<6}{'Agent':<20}{'Mean':<15}{'Std':<12}{'95% CI':<25}{'Seeds':<8}"
    logger.info(header)
    logger.info("-" * 80)
...
def save_results(results: dict, tests: dict, logger: logging.Logger) -> Path:
    ...
    output = {
        ...
        "statistical_tests": {k: {kk: convert(vv) for kk, vv in v.items()} for k, v in tests.items()},
    }
...
def create_comparison_plot(results: dict[str, dict], output_path: Path) -> None:
    ...
    for a in agents:
        ...
        ci_errors.append((results[a]["mean"] - ci_low + ci_high - results[a]["mean"]) / 2)
    
    fig, ax = plt.subplots(figsize=(12, 6))
    
    colors = plt.cm.viridis(np.linspace(0.2, 0.8, len(agents)))
    bars = ax.bar(agents, means, yerr=ci_errors, capsize=5, color=colors, edgecolor='black')
...
def main():
    ...
    parser.add_argument("--quick", action="store_true", help="Quick test (3 seeds, 100 episodes)")
    parser.add_argument("--tabular-only", action="store_true", help="Only run tabular methods")
    parser.add_argument("--deep-only", action="store_true", help="Only run deep RL methods")
