...
def main():
    ...
    parser.add_argument(
        "--algo",
        type=str,
        choices=["all", "baselines", "tabular", "deep", "q-learning", "sarsa", "dqn", "double-dqn", "dueling-dqn"],
        default="all",
    ...
    parser.add_argument(
        "--quick",
        action="store_true",
        help="Quick test run (100 episodes)"
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        "--offline",
        action="store_true",
        help="Run in offline mode (no wandb sync)"
    )
    parser.add_argument(
        "--project",
        type=str,
    ...
    if args.algo in ["all", "baselines"]:
        ...
        results["random"] = run_baseline_with_wandb(env, "random", args.episodes, logger)
        results["threshold"] = run_baseline_with_wandb(env, "threshold", args.episodes, logger)
