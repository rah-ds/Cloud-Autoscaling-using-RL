...
class DoubleDQNAgent:
    ...
    def learn(self, experiences, gamma):
        ...
        # Double DQN:
        # 1) Use local network to choose best action at next state
        with torch.no_grad():
            next_action_indices = self.qnetwork_local(next_states).argmax(1).unsqueeze(1)
            # 2) Use target network to value that action
            Q_targets_next = self.qnetwork_target(next_states).gather(1, next_action_indices)
