    ...
    for agent_key in agents_to_plot:
        ...
        if len(rewards) > smoothing_window:
            rolling_std = np.array([
                np.std(rewards[max(0, i-smoothing_window//2):i+smoothing_window//2])
...
def create_learning_curves_subplots(
    ...
    plt.close()
    print(f"  ✓ Learning curves (split): learning_curves_split.{style_config['format']}")
...
def create_ablation_line_plots(
    ...
    for ablation_name, ablation_data in ablations.items():
        ...
        ax.errorbar(
            param_values, rewards, yerr=stds,
            marker='o', markersize=style_config["markersize"],
            linewidth=style_config["linewidth"],
            capsize=4, color=COLORS["dqn"],
        )
        
        # Highlight best
        best_idx = np.argmax(rewards)
        ax.scatter(
            [param_values[best_idx]], [rewards[best_idx]],
            s=style_config["markersize"] * 20, marker='*',
            color=COLORS["dueling_dqn"], zorder=5, label='Best'
        )
...
def create_convergence_comparison(
    ...
    x = np.arange(len(agents))
    bars = ax.bar(x, convergence_episodes, color=colors, edgecolor='black', linewidth=0.5)
...
def create_summary_figure(
    ...
    for agent_key in ["threshold", "q_learning", "sarsa", "dqn", "double_dqn", "dueling_dqn"]:
    ...
    for agent_key in ["q_learning", "dqn", "dueling_dqn"]:
        if agent_key in results:
            ...
            if rewards:
                ...
                ax2.plot(
                    smoothed, label=AGENT_DISPLAY_NAMES.get(agent_key),
                    color=COLORS.get(agent_key), linewidth=1.5
                )
    ax2.set_xlabel("Episode")
    ax2.set_ylabel("Reward")
    ...
    ax3.bar(types, means, yerr=stds, color=['#EE3377', '#0077BB', '#009988'],
    ...
    if results:
        ...
        if best_agent:
            stats_text += f"Best Algorithm: {AGENT_DISPLAY_NAMES.get(best_agent, best_agent)}\n"
        ...
        if "baselines" in results and "threshold" in results["baselines"]:
            ...
            improvement = ((best_reward - threshold_reward) / abs(threshold_reward)) * 100
...
def main():
    ...
    parser.add_argument(
        "--style", choices=["paper", "poster", "presentation"],
        default="paper", help="Figure style preset"
    )
