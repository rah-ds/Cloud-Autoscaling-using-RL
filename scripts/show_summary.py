    ...
    for agent in ["dqn", "double_dqn", "dueling_dqn"]:
        if agent in results:
            ...
            if rewards:
                final_rewards = rewards[int(len(rewards) * 0.9):]
                mean_reward = sum(final_rewards) / len(final_rewards)
                display_name = agent.replace("_", " ").upper() if "dqn" in agent else agent.title()
...
def print_recommendations(results: dict, ablations: dict) -> None:
    ...
    if results:
        ...
        if deep_best > tabular_best and deep_best != float("-inf"):
            recommendations.append("• Deep RL agents outperform tabular methods - consider more training")
        elif tabular_best > deep_best and tabular_best != float("-inf"):
            recommendations.append("• Tabular methods performing well - deep RL may need more episodes")
    
    # Check ablation insights
    for name, data in ablations.items():
        results_list = data.get("results", [])
        if results_list and "learning_rate" in name:
            best = max(results_list, key=lambda x: x.get("metrics", {}).get("mean_reward_mean", float("-inf")))
            lr = best.get("params", {}).get("learning_rate", 0.1)
            if lr < 0.05:
                recommendations.append(f"• Low learning rate ({lr}) works best - patience pays off")
            elif lr > 0.2:
                recommendations.append(f"• High learning rate ({lr}) works best - fast adaptation needed")
