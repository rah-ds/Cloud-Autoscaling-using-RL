...
def compute_confidence_interval(data: List[float], window: int = 50, confidence: float = 0.95) -> tuple:
...
def plot_learning_curves(results: Dict[str, Any], save_path: Path) -> None:
    ...
    for algo, data in results.items():
        if isinstance(data, dict) and 'episode_rewards' in data and len(data['episode_rewards']) > 0:
    ...
    for algo, data in algos_with_data:
        ...
        ax.plot(range(50, 50 + len(smoothed)), smoothed, label=name, color=color, linewidth=2)
...
def plot_summary_dashboard(results: Dict[str, Any], save_path: Path) -> None:
    ...
    for algo, data in algos_with_data:
        ...
        name = ALGO_NAMES.get(algo, algo)
        smoothed = smooth_curve(rewards, window=50)
        ax1.plot(range(50, 50 + len(smoothed)), smoothed, label=name, color=color, linewidth=2)
    ...
    ax2.barh([algo_names[i] for i in sorted_idx], 
             [final_rewards[i] for i in sorted_idx],
             color=[colors_list[i] for i in sorted_idx], alpha=0.8)
    ...
    for algo, data in algos_with_data:
        ...
        name = ALGO_NAMES.get(algo, algo)
        window = 50
        variances = [np.var(rewards[max(0, i-window):i]) for i in range(window, len(rewards))]
        smoothed_var = smooth_curve(variances, 20)
        x = list(range(window, len(rewards)))
        min_len = min(len(x), len(smoothed_var))
        ax3.plot(x[:min_len], smoothed_var[:min_len], 
    ...
    for algo, data in algos_with_data:
        ...
        ax4.hist(final_rewards, bins=30, alpha=0.5, label=name, color=color, density=True)
    ...
    table_data = []
    headers = ['Algorithm', 'Episodes', 'Mean Reward', 'Std Dev', 'Best Reward', 'Final Reward', 'Improvement']
    ...
    # Add title
    fig.suptitle('Cloud Autoscaling RL - Training Summary Dashboard', 
...
def main():
    ...
    plot_algorithm_comparison(results, output_dir / f"algorithm_comparison_{timestamp}.png")
    plot_convergence_analysis(results, output_dir / f"convergence_analysis_{timestamp}.png")
