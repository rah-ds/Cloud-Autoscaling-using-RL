...
class AblationStudy:
    ...
    def get_best_config(self, metric: str = "mean_reward", higher_is_better: bool = True) -> AblationResult:
        """Get the configuration with the best performance."""
        if not self.results:
            raise ValueError("No results in study")
        
        key_func = lambda r: r.metrics.get(metric, float('-inf') if higher_is_better else float('inf'))
        return max(self.results, key=key_func) if higher_is_better else min(self.results, key=key_func)
    ...
    def to_dataframe(self):
        ...
        for result in self.results:
            row = {"config_name": result.config.name, **result.config.params, **result.metrics}
            rows.append(row)
    ...
    with tqdm(total=total_runs, desc=f"Ablation: {param_name}") as pbar:
        for value in param_values:
            ...
            for seed in range(n_seeds):
                ...
                agent, train_metrics = train_fn(env, agent, n_episodes=n_episodes, verbose=False)
            ...
            result = AblationResult(
                ...
                training_curves={"episode_rewards": [np.mean([c[i] if i < len(c) else c[-1] for c in seed_curves]) 
    ...
    with tqdm(total=total_runs, desc="Component Ablation") as pbar:
        for config_name, param_mods in all_configs:
            ...
            config = AblationConfig(
                name=config_name,
                params=params,
                description=f"Without {config_name}" if config_name != "full_model" else "Full model"
            )
...
def run_grid_ablation(
    ...
    study = AblationStudy(
        name="grid_ablation",
        description=f"Grid search over: {list(param_grid.keys())}"
    )
    ...
    logger.info(f"Running grid ablation: {len(combinations)} combinations x {n_seeds} seeds = {total_runs} runs")
...
def plot_ablation_results(
    ...
    bars = ax.bar(names, values, yerr=errors, capsize=5, color=colors, edgecolor='white', linewidth=1.5)
...
def plot_ablation_heatmap(
    ...
    ax.set_title(f"Ablation Heatmap: {param1} vs {param2}", fontsize=14, fontweight='bold')
...
def create_ablation_table(
    ...
    # Build table
    headers = ["Configuration"] + [m.replace("_mean", "").replace("_", " ").title() for m in metrics]
    
    # Calculate column widths
    col_widths = [max(len(h), max(len(r.config.name) for r in results)) for h in [headers[0]]]
...
if __name__ == "__main__":
    ...
    # Create mock study
    study = AblationStudy(
        name="learning_rate_demo",
        description="Demo ablation study for learning rates"
    )
