...
def plot_learning_curve(
    ...
    ax.plot(smooth_episodes, smoothed, color=color, linewidth=2, label=f"Smoothed (w={window})")
...
def plot_multi_seed_learning_curve(
    ...
    # Plot mean
    ax.plot(episodes, mean, color=color, linewidth=2, label=f"Mean (n={len(all_rewards)})")
...
def plot_algorithm_comparison_bar(
    ...
    stds = [m.get(f"std_{metric_name.replace('mean_', '')}", 0) for m in metrics.values()]
...
def plot_hyperparameter_heatmap(
    ...
    param1_values = sorted(set(r[param1_name] for r in results.values() if param1_name in r))
    param2_values = sorted(set(r[param2_name] for r in results.values() if param2_name in r))
...
def plot_policy_heatmap(
    ...
    for i, ax in enumerate(axes):
        trend_name = state_labels["trend"][i] if i < len(state_labels["trend"]) else f"Trend {i}"
        
        im = ax.imshow(policy[:, :, i], cmap=cmap, aspect="auto", vmin=0, vmax=len(action_names)-1)
        ...
        for y in range(policy.shape[0]):
            for x in range(policy.shape[1]):
                ...
                ax.text(x, y, action_names[action][:2], ha="center", va="center", fontsize=8)
...
def plot_training_summary(
    ...
    if rewards:
        ...
        smoothed = smooth_curve(rewards, window=window)
        ax.plot(np.arange(len(smoothed)) + window//2, smoothed, color="blue", linewidth=2)
    ...
    if sla:
        ...
        ax.plot(np.arange(len(smoothed)) + window//2, smoothed, color="red", linewidth=2)
    ax.set_xlabel("Episode")
    ...
    if costs:
        ...
        ax.plot(np.arange(len(smoothed)) + window//2, smoothed, color="green", linewidth=2)
    ax.set_xlabel("Episode")
    ax.set_ylabel("Cost")
    ...
    if lengths:
        ...
        ax.plot(np.arange(len(smoothed)) + window//2, smoothed, color="purple", linewidth=2)
    ax.set_xlabel("Episode")
...
if __name__ == "__main__":
    ...
    multi_seed = [
        (np.cumsum(np.random.randn(n_episodes) * 10) - 1000).tolist()
        for _ in range(5)
    ]
    ...
    eval_metrics = {
        "Q-Learning": {"mean_reward": -500, "std_reward": 50, "mean_sla_violations": 2.5, "mean_cost": 150, "mean_utilization": 0.72},
        "SARSA": {"mean_reward": -600, "std_reward": 60, "mean_sla_violations": 3.0, "mean_cost": 140, "mean_utilization": 0.68},
        "Random": {"mean_reward": -1500, "std_reward": 100, "mean_sla_violations": 10.0, "mean_cost": 200, "mean_utilization": 0.45},
    }
    
    fig = plot_metrics_dashboard(
        eval_metrics,
        save_path="artifacts/plots/demo_dashboard.png"
    )
    plt.close(fig)
