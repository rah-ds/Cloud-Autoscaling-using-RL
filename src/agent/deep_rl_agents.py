...
class DuelingDQNAgent(DQNAgent):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.q_network = DuelingQNetwork(state_dim, action_dim, hidden_dims).to(
            self.device
        )
        self.target_network = DuelingQNetwork(state_dim, action_dim, hidden_dims).to(
            self.device
        )
        self.target_network.load_state_dict(self.q_network.state_dict())
...
def train_dqn(
    ...
    for episode in tqdm(
        range(n_episodes), desc=f"{agent_name} Training", disable=not verbose
    ):
...
if __name__ == "__main__":
    ...
    print(
        f"DuelingQNetwork: {sum(p.numel() for p in dueling_net.parameters())} parameters"
    )
