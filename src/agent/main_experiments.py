    ...
    with tqdm(total=total_configs, desc="Exploration Rate Sweep", leave=False) as pbar:
        for eps_init in epsilon_values:
            for eps_decay in epsilon_decays:
                ...
                logger.info(f"[{config_num}/{total_configs}] Configuration: {config_name}")
            ...
            logger.info(f"  Result: reward={q_eval['mean_reward']:.2f}, sla={q_eval['mean_sla_violations']:.2f}")
            pbar.update(1)
...
def experiment_3_episode_lengths(workload_data, episode_lengths=[500, 1000, 2000]):
    ...
    for i, n_episodes in tqdm(enumerate(episode_lengths), total=len(episode_lengths), desc="Episode Length Sweep", leave=False):
        logger.info(f"[{i+1}/{len(episode_lengths)}] Training with {n_episodes} episodes")
...
def experiment_4_learning_rates(workload_data, n_episodes=1000):
    ...
    for i, lr in tqdm(enumerate(learning_rates), total=len(learning_rates), desc="Learning Rate Sweep", leave=False):
        ...
        logger.info(f"  Q-Learning: reward={q_eval['mean_reward']:.2f}, sla={q_eval['mean_sla_violations']:.2f}")
        logger.info(f"  SARSA: reward={sarsa_eval['mean_reward']:.2f}, sla={sarsa_eval['mean_sla_violations']:.2f}")
